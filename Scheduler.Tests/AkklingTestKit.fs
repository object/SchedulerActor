...
let inline ignoreMessages (tck: Tck) (predicate: obj -> bool) : unit = tck.IgnoreMessages(fun x -> predicate(x))
...
let inline within (tck: Tck) (max: TimeSpan) (fn: unit->'t) : 't = tck.Within(max, fun x -> fn(x))
...
let inline expectEvent (n:int) (applier: IEventFilterApplier) (fn: unit->'t): 't = applier.Expect(n, fun x -> fn(x))
let inline expectEventWithin (timeout: TimeSpan) (n:int) (applier: IEventFilterApplier) (fn: unit->'t): 't = applier.Expect(n, timeout, fun x -> fn(x))
